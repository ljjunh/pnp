...
describe('Amenity 컴포넌트 테스트', () => {
  ...
  test('더 표시 버튼 클릭 시 모든 태그 렌더링 테스트', () => {
    ...
    // 필수 편의시설 태그 확인
    const essentialTags = screen.getAllByText(
      /와이파이|주방|세탁기|건조기|에어컨|난방|업무 전용 공간|TV|헤어드라이어|다리미/,
    );
    expect(essentialTags).toHaveLength(10);

    // 특징 편의시설 태그 확인
    const characteristicTags = screen.getAllByText(
      /수영장|대형 욕조|무료 주차 공간|전기차 충전시설|아기 침대|킹사이즈 침대|헬스장|바베큐 시설|조식|벽난로|흡연 가능/,
    );
    expect(characteristicTags).toHaveLength(11);

    // 위치 편의시설 태그 확인
    const locationTags = screen.getAllByText(/해변에 인접|수변/);
    expect(locationTags).toHaveLength(2);

    // 안전 편의시설 태그 확인
    const safetyTags = screen.getAllByText(/화재경보기|일산화탄소 경보기/);
