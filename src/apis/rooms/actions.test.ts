...
describe('Scrap Action Test', () => {
  ...
  describe('deleteScrap', () => {
    test('성공적으로 스크랩 삭제에 성공한다', async () => {
      (auth as jest.Mock).mockResolvedValue({ user: { id: 1 } });

      const result = await deleteScrap(204);

      expect(result.success).toBe(true);
      expect(result.status).toBe(204);
      expect(revalidateTag).toHaveBeenCalledWith(CACHE_TAGS.ROOMS.SCRAP(204));
      expect(revalidateTag).toHaveBeenCalledTimes(1);
    });
    ...
    test('로그인 하지 않은 경우 api호출을 하지 않고 401 응답 객체를 반환한다', async () => {
      ...
      const result = await deleteScrap(1);
    ...
    test('서버에서 401응답이 오는 경우 401 응답 객체를 반환한다', async () => {
      (auth as jest.Mock).mockResolvedValue({ user: { id: 1 } });
      const result = await deleteScrap(401);
    ...
    test('서버 에러 응답에 message가 없는 경우 기본 메시지를 반환한다', async () => {
      ...
      const result = await deleteScrap(502);

      expect(result.success).toBe(false);
      expect(result.status).toBe(500);
      expect(result.message).toBe('스크랩 삭제에 실패했습니다. 잠시 후 다시 시도해 주세요.');
    });

    test('서버에서 에러 응답이 온 경우 서버에서 온 응답 객체(success, status, message)를 그대로 반환한다', async () => {
      (auth as jest.Mock).mockResolvedValue({ user: { id: 1 } });

      const result = await deleteScrap(500);
    ...
    test('네트워크 에러 발생 시 500 응답 객체를 반환한다', async () => {
      ...
      const result = await deleteScrap(501);
